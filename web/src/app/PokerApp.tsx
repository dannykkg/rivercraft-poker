  ...
  return <section className="relative min-h-[680px] overflow-hidden rounded-[28px] border border-white/8 bg-[#0c1513] p-4 shadow-2xl shadow-black/30 sm:p-8 lg:min-h-[calc(100vh-112px)]">
    ...
    <div className="relative mx-auto mt-24 aspect-[1.72/1] w-[82%] max-w-5xl rounded-[46%] border-[10px] border-[#271e19] bg-[#123f32] shadow-[inset_0_0_0_2px_rgba(255,255,255,.08),inset_0_0_70px_rgba(0,0,0,.48),0_34px_70px_rgba(0,0,0,.42)] sm:mt-28 sm:w-[88%]">
      ...
      {tableSeats.map((player, visualSeat) => {
        ...
        return player ? <div key={player.id} data-table-seat={visualSeat + 1} style={{ left: `${position.left}%`, top: `${position.top}%` }} className={`absolute flex min-w-[118px] -translate-x-1/2 -translate-y-1/2 items-center gap-2 rounded-2xl border px-2.5 py-2 shadow-xl ring-1 ring-inset ring-white/10 backdrop-blur sm:min-w-[136px] sm:px-3 sm:py-2.5 ${player.kind === "human" ? "border-emerald-200/65 bg-[#215248]" : "border-sky-100/30 bg-[#293640]"}`}>
    ...
    <section className="relative flex min-h-[720px] flex-col overflow-hidden rounded-[28px] border border-white/8 bg-[#0b1311] p-4 lg:min-h-[calc(100vh-104px)] lg:p-6">
      ...
      <div className="relative z-10 mx-auto mt-16 aspect-[1.72/1] w-[82%] max-w-[1120px] rounded-[46%] border-[10px] border-[#281e18] bg-[#124334] shadow-[inset_0_0_0_2px_rgba(255,255,255,.08),inset_0_0_90px_rgba(0,0,0,.5),0_36px_70px_rgba(0,0,0,.46)] sm:mt-20 sm:w-[90%]">
        ...
        {tableSeats.map((player, visualSeat) => {
          ...
          return <div key={player.id} data-table-seat={visualSeat + 1} data-player-id={player.id} data-seat-state={seatState} style={{ left: `${position.left}%`, top: `${position.top}%` }} className={`absolute min-w-[132px] -translate-x-1/2 -translate-y-1/2 rounded-2xl border p-2.5 shadow-xl ring-1 ring-inset ring-white/10 backdrop-blur transition sm:min-w-[156px] sm:p-3 ${winner ? "winner-seat border-amber-200/80 bg-[#3a3219] shadow-[0_0_34px_rgba(253,230,138,.32)] ring-2 ring-amber-200/30" : displayEliminated ? "border-dashed border-zinc-700 bg-[#171b1e] grayscale shadow-none" : player.folded ? "border-dashed border-zinc-400/50 bg-[#24282c] grayscale shadow-none" : isCurrent ? "border-amber-100 bg-[#494125] shadow-[0_0_34px_rgba(253,230,138,.34)] ring-2 ring-amber-200/35" : isHero ? "border-emerald-100/75 bg-[#215248] shadow-[0_14px_32px_rgba(0,0,0,.48),0_0_20px_rgba(52,211,153,.18)]" : "border-sky-100/35 bg-[#293640] shadow-[0_14px_32px_rgba(0,0,0,.48),0_0_16px_rgba(125,211,252,.08)]"}`}>
